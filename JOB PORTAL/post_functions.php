...
{
	global $conn;
	$sql = "SELECT name FROM company WHERE id=$user_id";
	$result = mysqli_query($conn, $sql);
	if ($result) {
		// return username
...
function createPost($request_values){
		global $conn, $errors, $title, $topic_id, $body;
...
*/
    ...
    if (isset($_GET['publish']) || isset($_GET['unpublish'])) {
        ...
        if (isset($_GET['publish'])) {
			...
			publishPost($post_id, $message);
        } else if (isset($_GET['unpublish'])) {
            $message = "Post successfully unpublished";
            $post_id = $_GET['unpublish'];
			unpublishPost($post_id, $message);
        } 
    }
    function publishPost($post_id, $message)
    {
    ...
    function unpublishPost($post_id, $message)
    {
        $conn = mysqli_connect("localhost", "root", "", "job_portal");
